...
| [terminal/explanation/session-resume.md](terminal/explanation/session-resume.md) | Explanation | How a rate-limited Claude Code session resumes in place via the `StopFailure` and `Stop` hooks, the status line quota cache, and `cc-resume`. |
