...
On request the installer wires up the Claude Code layer. This is **opt-in**: `install.sh` prompts once (reading from `/dev/tty`), or runs without prompting when `INSTALL_CLAUDE=1` is set; with no tty and no env var it is skipped. It covers three things, and grows as more Claude tooling is added:
...
- **Session resume** — symlinks the `StopFailure` hook, the quota-caching status line, and the `cc-resume` watcher, then merges their entries into `~/.claude/settings.json`. Together they resume a rate-limited session by typing into its herdr pane once the limit lifts. See [Session resume](../explanation/session-resume.md).
