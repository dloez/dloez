...
- **`verify.sh`** asserts deps present, starship + fzf + herdr + nvim + kickstart config + plugins + all symlinks in place, zsh is the default shell, the config sources cleanly, that the pure-zsh instant paint matches `starship-fast.toml` byte-for-byte across a directory matrix (so the async swap never jumps — see [Async prompt design](../explanation/async-prompt.md)), and that a **second** `install.sh` run is idempotent (reuses the existing starship). When `INSTALL_CLAUDE=1`, it also asserts the `~/.claude/skills/<name>` symlinks for each skill in `essential-skills.txt`, plus the learning plugin in the clone and the `.learning-enabled` marker. `[repo]` defaults to the parent of the script.
