...
1. Run the bootstrap:
   ...
   9. Asks whether to set up the Claude Code layer — Claude Code itself, the essential skills (`.claude/skills/essential-skills.txt`), the session-resume runtime and its autostart, and the nvim learning loop (opt-in). Answer `y` at the prompt, or set `INSTALL_CLAUDE=1` to run it without prompting; add `INSTALL_CLAUDE_CLI=0` to skip the Claude Code download. Skipped silently when there is no terminal to prompt on. See the [Claude Code setup](../reference/layout-and-testing.md) and [Enable session resume](enable-session-resume.md).
