...
- **Claude Code session resume** — a rate-limited session is picked back up where it stopped: a `StopFailure` hook records the stop, the status line caches the quota reset time, and `cc-resume` types the resume text into that herdr pane once the limit lifts. The watcher is supervised by systemd (Linux/WSL) or launchd (macOS), so it starts at login and survives a reboot. No model is called to decide any of it. See [Session resume](explanation/session-resume.md).
