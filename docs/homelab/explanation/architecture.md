...
- **Node CPU limits are deliberately kept under the core count.** worker1 has 4 cores (3950m allocatable) and the chain clients are the only workloads that can genuinely saturate it, so nethermind is capped at 2 and the beacon at 1 — leaving headroom for the kubelet and Longhorn's instance manager. Sum of limits across the node still exceeds capacity, which is fine for idle controllers but is worth checking before adding another heavy workload.
