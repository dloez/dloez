...
- **Snap-sync progress does not survive a nethermind restart — avoid restarting it mid-sync.** Observed on 2026-08-06: a rolling restart to change resource limits dropped `Snap State Ranges (Phase 1)` from 31.56% back to ~4%, with `nethermind_snap_synced_accounts` falling from 3.5 M to 0.5 M, even though the pivot block was unchanged and the volume was intact. Downloaded state ranges are staged rather than committed until the phase completes, so anything that recycles the pod during initial sync throws that work away. Batch config changes to the gnosis stack and apply them in one reconcile, or accept the cost knowingly — once synced this no longer applies, because the node is then following chain head rather than rebuilding state.
